...
if __name__ == "__main__":
    ...
    parser.add_argument('--device', type=int, default=0)  # 设备编号
    parser.add_argument('--num_layers', type=int, default=2)  # 模型层数
    parser.add_argument('--train_ratio', type=float, default=0.1)  # 训练集比例
    parser.add_argument('--p_encoder', type=str, default="P_GIN",choices=['P_GIN', 'SAGE', 'GAT'])  # structure 编码器类型
    parser.add_argument('--s_encoder', type=str, default="S_GIN")  # proximity 编码器类型
    parser.add_argument('--num_workers', type=int, default=1)  # 数据加载的线程数
    parser.add_argument('--dataset', type=str, default='pokec_n')  # 数据集名称
    parser.add_argument('--sens_attr', type=str, default='region')  # 敏感属性名称
    ...
    for i in range(3):  # 重复实验3次以计算平均值和方差
        print(f'====exp:{i}====')
